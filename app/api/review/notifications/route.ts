...
async function proxyNotifications(url: string, init: RequestInit) {
  try {
...
export async function GET(request: NextRequest) {
  const targetUrl = new URL("/api/review/notifications", getMainPlatformBaseUrl());
  ...
  return proxyNotifications(targetUrl.toString(), { method: "GET" });
}

export async function POST(request: NextRequest) {
  const body = await request.json().catch(() => ({}));
  return proxyNotifications(`${getMainPlatformBaseUrl()}/api/review/notifications`, {
    method: "POST",
    headers: {
